...
SUBSET = "UFPR05"
ROOT_DB = "/Users/vanessanamirembe/Desktop/CarNet/training_files/PKLot.v2-640.yolov5pytorch"
...
class Main(object):
    ...
    def create_files(self):
        if self.all_images_as_string is None:
          print("Error: No images found. Aborting file creation.")
        return
    ...
    def convert_string_to_np_array(self, attr):
        ...
        filename = self.train_txt if attr == 'train' else self.test_txt
        train_data, labels = [], []
        height, width, channels = self.get_input_shape()
        with open(filename, 'r') as f:
            for line in f:
                img_path, label = line.split()
                img = Image.open(os.path.join(self.root_subset, img_path))
